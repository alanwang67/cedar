...
fn schema_file() -> json_schema::NamespaceDefinition<RawName> {
    serde_json::from_value(serde_json::json!(
            {
            "entityTypes": {
                "Disk": {
...
mod templates {
    ...
    #[test]
    fn template_resource_slot_in_condition_safe() {
        let s = schema_file();
        ...
        let templates = [
            ...
            r#"permit(principal == ?principal, action == Action::"Access", resource in ?resource) when { ?resource.brand == "San Disk" };"#,
        ];
    ...
    #[test]
    fn template_resource_slot_in_condition_fails() {
        let s = schema_file();
