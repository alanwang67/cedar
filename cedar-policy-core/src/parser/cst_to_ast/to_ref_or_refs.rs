...
impl RefKind for SingleEntity {
    ...
    fn create_slot(_: Option<Id>, loc: Option<&Loc>) -> Result<Self> {
        Err(ToASTError::new(
            ToASTErrorKind::wrong_entity_argument_one_expected(
                err::parse_errors::Ref::Single,
    ...
    fn to_ref_or_refs<T: RefKind>(
        ...
        match prim {
            cst::Primary::Slot(s) => {
                ...
                // support slots.
                let slot_ref = T::create_slot(None, self.loc.as_loc_ref())?; // The value in the first parameter here has no semantic meaning it is just used as a check
