...
impl RefKind for EntityReference {
    ...
    fn create_slot(id: Option<Id>, loc: Option<&Loc>) -> Result<Self> {
        Ok(EntityReference::Slot(id, loc.cloned()))
    }
    ...
    fn to_ref_or_refs<T: RefKind>(
        ...
        match prim {
            cst::Primary::Slot(s) => {
                ...
                let slot_ref = T::create_slot(None, self.loc.as_ref())?; // TODO: Currently the only allowed slots are principal & resource, however this will change with generalized slots
