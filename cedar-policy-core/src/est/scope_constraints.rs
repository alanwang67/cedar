    ...
    fn from(constraint: ast::PrincipalOrResourceConstraint) -> PrincipalConstraint {
        match constraint {
            ...
            ast::PrincipalOrResourceConstraint::Eq(ast::EntityReference::Slot(id, _)) => {
                // Todo: double check if this conversion makes sense after you read the surrounding code
                let slot = match id {
...
impl TryFrom<PrincipalConstraint> for ast::PrincipalOrResourceConstraint {
    ...
    fn try_from(
        ...
        match constraint {
            ...
            PrincipalConstraint::Eq(EqConstraint::Slot { slot }) => {
                if slot.is_principal() || slot.is_generalized_slot() {
                    // ToDo: Double check if this captures everything
                    Ok(ast::PrincipalOrResourceConstraint::Eq(
                        ast::EntityReference::Slot(slot.extract_id(), None),
                    ))
                } else {
